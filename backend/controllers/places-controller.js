...
const createPlace = async (req, res, next) => {
  ...
  const createdPlace = new Place({
    ...
    image: 'https://i.redd.it/9w9wo4778k261.png',
    creator,
  });
  ...
  try {
    ...
    const err = new HttpError(' creating place fail, plase try again', 500);
    return next(err);
  }
  console.log(user);

  if (!user) {
    const err = new HttpError('could not fin user for provided id', 404);
  ...
  try {
    ...
    const err = new HttpError('createing place failed, place try again', 500);
    return next(err);
  }
